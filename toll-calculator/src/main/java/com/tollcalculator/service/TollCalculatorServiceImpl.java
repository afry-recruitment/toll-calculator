...
import com.tollcalculator.model.CityTollRate;
...
public class TollCalculatorServiceImpl implements TollCalculatorService {
     ...
     **/
    @Override
    public TollCalculatorResponse getTollFeeAmount(String cityName, VehicleType vehicleType, List<Date> dates) {
        City cityObject = cityRepository.findByName(cityName).get();

        // calculate toll for exempt vehicle
        if (IsTollFreeVehicle(cityObject.getTollExemptVehicles(), vehicleType)) {
            LOG.debug("Requested vehicle {} has free toll", vehicleType.getType());
            return TollCalculatorResponse.builder().tollFeeAmount(new BigDecimal(0)).build();
        }
        ...
        Integer singleChargeIntervalInMin = cityObject.getCityTollChoice().getSingleChargeIntervalInMin();
        ...
        Integer maxTollPerDay = cityObject.getCityTollChoice().getMaxTollPerDay();
        if (totalFee > maxTollPerDay) {
            LOG.debug("Eligible for Capping Requested vehicle {} toll fees to max fees per day", vehicleType.getType());
            totalFee = maxTollPerDay;
        }
        return TollCalculatorResponse.builder().tollFeeAmount(new BigDecimal(totalFee)).build();
    }
    ...
    public int getTollFee(final Date date, City city) {
        ...
        int charge = getTollFeeByTariffAndDate(date, city.getCityTollRates());
        return charge;
    }

    private int getTollFeeByTariffAndDate(Date date, Set<CityTollRate> tariffs) {
        BigDecimal totalFee = new BigDecimal(0);
        if (tariffs == null || tariffs.isEmpty()) return totalFee.intValue();

        for (CityTollRate tariffEntity : tariffs) {
    ...
    private boolean IsTollFreeVehicle(Set<Vehicle> tollExemptVehicles, VehicleType vehicle) {
        if (tollExemptVehicles == null) return false;
        if (tollExemptVehicles.stream().filter(tollExemptVehicle ->
                tollExemptVehicle.getName().equalsIgnoreCase(vehicle.getType())).count() > 0) return true;
        return false;
    }
