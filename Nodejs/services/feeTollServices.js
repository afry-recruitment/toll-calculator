...
function getFeeForTime(passingTime, feeTime) {
    for (const interval of feeTime.intervals) {
        ...
        const toTime = addToTime(fromTime, Math.abs(fromHour-toHour) , Math.abs(fromMinute-toMinute) );
        if (isBetweenIntervals(passingTime, fromTime, toTime)) return feeTime.amount;
    }
    return false;
}
...
function addToTime(time,hour,minute){
    ...
    tempDate.setTime( tempDate.getTime() + (hour*60*60*1000) + (minute*60*1000)  )
    return tempDate;
