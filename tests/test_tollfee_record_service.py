...
from src.services.tollfee_record_sevice import _calculate_toll_fee
...
def test_calculate_toll_fee_no_days():
    ...
    assert _calculate_toll_fee(start_time, end_time, hourly_rates) == 34.0


def test_calculate_toll_fee_one_day():
    start_time = datetime(2022, 1, 1, 8, 0, 0)
    end_time = datetime(2022, 1, 2, 10, 0, 0)
    assert _calculate_toll_fee(start_time, end_time, hourly_rates) == 94.0


def test_calculate_toll_fee_multiple_days():
    start_time = datetime(2022, 1, 1, 8, 0, 0)
    end_time = datetime(2022, 1, 5, 10, 0, 0)
    assert _calculate_toll_fee(start_time, end_time, hourly_rates) == 274.0


def test_calculate_toll_fee_rounding():
    start_time = datetime(2022, 1, 1, 8, 0, 0)
    end_time = datetime(2022, 1, 1, 11, 30, 0)
    assert _calculate_toll_fee(start_time, end_time, hourly_rates) == 60.0
